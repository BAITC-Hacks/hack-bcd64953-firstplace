  ...
  if p_role = 'business' then
    ...
    if jsonb_typeof(p_data->'student') = 'object' then
      update public.student_profiles set
        ...
        skills = case when p_data->'student' ? 'skills' then
        ...
        portfolio_url = case when p_data->'student' ? 'portfolio_url'
          then p_data->'student'->>'portfolio_url' else portfolio_url end where profile_id = p.id;
    end if;
...
begin
  ...
  if new.status = 'published' and new.readiness_score <> 100 then
    raise exception 'Application is incomplete' using errcode = '23514';
  end if;
  return new;
end;
$$;
...
-- PostgreSQL grants PUBLIC execute by default. Explicitly close every entry point.
